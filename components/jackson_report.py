...
def build_jackson_report(roads, results: dict) -> bytes:
    ...
    story.append(Paragraph(
        ...
        f"representing ${results['spent']:,.0f} in investment and an illustrative {results['risk_reduction_percent']:.0f}% reduction in portfolio risk.",
        body,
    ))
    ...
    for _, road in results["roads"].iterrows():
        investment_rows.append([
            str(int(road["Priority Rank"])), road["Road Name"], road["Treatment"],
            f"${road['Estimated Cost']:,.0f}", f"{road['PCI']:.0f}", f"{road['Risk Score']:.0f}",
        ])
    investment_table = Table(investment_rows, repeatRows=1, colWidths=[0.45 * inch, 1.45 * inch, 1.25 * inch, 1.05 * inch, 0.5 * inch, 0.5 * inch])
    ...
    story.append(Paragraph(
        "Priorities use Paventra's transparent Phase 1 rules: PCI/condition, traffic exposure, asset age, and freeze-thaw exposure. "
        "The planning impact assumes selected projects remove their current risk-score points. It is not an engineering forecast or construction estimate.",
        body,
    ))
