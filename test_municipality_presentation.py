...
from components.jackson_report import (
...
from pilot.municipality_registry import ACTIVE_MUNICIPALITY, MUNICIPALITIES
...
class MunicipalityPresentationTests(unittest.TestCase):
    def test_generic_scenario_selection_is_municipality_neutral(self):
        ...
        with self.assertRaisesRegex(KeyError, "Unknown municipality scenario"):
        ...
        for slug, config in MUNICIPALITIES.items():
            with self.subTest(municipality=slug):
                ...
                self.assertEqual(
                    ...
                    f"{config.name} Transportation Investment Scenario",
                )
                self.assertEqual(
    ...
    def test_streamlit_full_flow_for_both_municipalities(self):
        script = """
import os
