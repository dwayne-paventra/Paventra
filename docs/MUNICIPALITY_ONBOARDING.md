...
The output is written only after validation and uses deterministic canonical column order. Review identifiers, road names, coordinates, PCI, treatment, cost, and source labels. Existing files are protected. Replace one only after deliberate review:
...
The dashboard continues to load the mapped source through the same canonical pipeline; the review export is not a second analytics path.
